...
In-paper anchor: `paper/BLOCH_BEILINSON_CONJECTURES_PREPRINT.md` (`BB_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of admissible motivic filtrations, regulator compatibilities, and cycle-class packages through a multi-lane motivic super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BB_G3` closes when `0.8038585209003215` provides compactness/no-collapse control: normalized near-failure families stay precompact and restart spacing does not collapse.
This is the first-failure capture contribution to `M_BB`.
...
If near-failure extraction and no-Zeno spacing hold, then `BB_G3` prevents hidden drift before rigidity is applied.
