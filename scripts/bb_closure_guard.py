#!/usr/bin/env python3
"""Canonical-lane closure guard for the bloch-beilinson conjectures super-repo."""
...
PRIMARY_KEYS = ('kappa_filtration', 'sigma_regulator', 'kappa_compact', 'rho_rigidity', 'motivic_transfer')
...
def compute_report(data,sigma_star,strict):
    ...
    gates={f'BB_G{i}':('PASS' if p else 'FAIL') for i,p in enumerate(gate_pass, start=1)}
    gates['BB_G6']='PASS' if coh_pass else 'FAIL'
    gates['BB_GM']='PASS' if gm_pass else 'FAIL'
    blockers={name:([] if status=='PASS' else ['missing/nonpositive or not theorem-level']) for name,status in gates.items() if name not in ('BB_G6','BB_GM')}
    blockers['BB_G6']=[] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['BB_GM']=[] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates={'G1':gates['BB_G1'],'G2':gates['BB_G2'],'G3':gates['BB_G3'],'G4':gates['BB_G4'],'G5':gates['BB_G5'],'G6':'NA','GCoh':gates['BB_G6'],'GM':gates['BB_GM']}
    normalized_blockers={'G1':blockers['BB_G1'],'G2':blockers['BB_G2'],'G3':blockers['BB_G3'],'G4':blockers['BB_G4'],'G5':blockers['BB_G5'],'G6':[],'GCoh':blockers['BB_G6'],'GM':blockers['BB_GM']}
    report={'meta':{'computed_at_utc':dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(),'framework':'bloch_beilinson_conjectures' },'schema':{'normalized_gate_keys':['G1','G2','G3','G4','G5','G6','GCoh','GM'],'g6_policy':'NA means this framework has no standalone pre-coherence G6 gate'},'lane':{'canonical_theorem_lane':'manifold_constrained','active_lane':'manifold_constrained'},'inputs':{**vals,'eps_coh':eps_coh,'sigma_star_can':sigma_star},'derived':{'strict_margin':margin,'strict_coh_zero':bool(strict)},'gates':gates,'blockers':blockers,'normalized':{'gates':normalized_gates,'blockers':normalized_blockers}}
